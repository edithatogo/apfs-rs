...
enum Command {
    ...
    FixtureManifestCheck { manifest: PathBuf },
    /// Compare inspect JSON to a fixture manifest and write feedback artifacts.
    RealFixtureFeedback { inspect_json: PathBuf, manifest_json: PathBuf, out_dir: PathBuf },
    /// Promote feedback JSON into generated Codev and Conductor task stubs.
    PromoteFeedback { feedback_json: PathBuf, out_dir: PathBuf },
    /// Promote Cargo output into generated Codev/Conductor triage tasks.
    CargoTriage { cargo_log: PathBuf, out_dir: PathBuf },
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::RealFixtureFeedback { inspect_json, manifest_json, out_dir } => real_fixture_feedback(&inspect_json, &manifest_json, &out_dir),
        Command::PromoteFeedback { feedback_json, out_dir } => promote_feedback(&feedback_json, &out_dir),
        ...
        Command::LocalEnvDoctor { json } => {
            if let Some(path) = json {
                ...
                run_python_tool("tools/local_env_doctor.py", &["--json", path_string.as_str()])
            } else {
                run_python_tool("tools/local_env_doctor.py", &[])
            }
        },
        Command::HandoffStatus { write } => {
            if write { run_python_tool("tools/handoff_status.py", &["--write"]) } else { run_python_tool("tools/handoff_status.py", &[]) }
        },
        Command::RepoManifest { write } => {
            if write { run_python_tool("tools/repo_manifest.py", &["--write"]) } else { run_python_tool("tools/repo_manifest.py", &[]) }
        },
        Command::KnownRiskCheck => run_python_tool("tools/known_risk_check.py", &[]),
        Command::LocalCompileLoop { execute } => {
            if execute { run_python_tool("tools/local_compile_loop.py", &["--execute"]) } else { run_python_tool("tools/local_compile_loop.py", &[]) }
        },
        ...
        Command::CurrentEnvironmentInventory => run_python_tool("tools/current_environment_inventory.py", &[]),
        ...
        Command::SyntheticNegativeFixtures => run_python_tool("tools/synthetic_negative_fixture_generator.py", &[]),
        ...
        Command::DependencyPolicyAudit => run_python_tool("tools/dependency_license_policy_check.py", &[]),
...
fn require_non_empty(path: impl AsRef<Path>) -> Result<()> {
    ...
    let text = fs::read_to_string(path).with_context(|| format!("read required file {}", path.display()))?;
...
fn validate_capability_safety_gate_links() -> Result<()> {
    let capabilities_text = fs::read_to_string("codev/resources/capabilities.yaml").context("read capabilities registry")?;
    let gates_text = fs::read_to_string("codev/resources/safety-gates.yaml").context("read safety-gate registry")?;
    let capabilities: serde_yaml::Value = serde_yaml::from_str(&capabilities_text).context("parse capabilities registry")?;
    let gates: serde_yaml::Value = serde_yaml::from_str(&gates_text).context("parse safety-gate registry")?;
    ...
    for (capability_id, capability) in capability_map {
        let capability_id = capability_id.as_str().unwrap_or("<non-string capability id>");
        let Some(gates) = yaml_get(capability, "safety_gates").and_then(serde_yaml::Value::as_sequence) else {
        ...
        for gate in gates {
            ...
            if !gate_map.contains_key(&gate_key) {
                anyhow::bail!("capability {capability_id} references undefined safety gate {gate_name}");
            }
        }
    }
    Ok(())
...
fn scan_for_unsafe_blocks() -> Result<()> {
    for entry in WalkDir::new("crates").into_iter().chain(WalkDir::new("xtask").into_iter()) {
        let entry = entry?;
        ...
        for (line_no, line) in text.lines().enumerate() {
            ...
            let detector_implementation_line = path.ends_with("xtask/src/main.rs")
                && (trimmed.contains("trimmed.starts_with") || trimmed.contains("trimmed.contains"));
            ...
            if suspicious && !allowed_lint && !detector_implementation_line {
                anyhow::bail!("unreviewed unsafe-looking code at {}:{}", path.display(), line_no + 1);
            }
        }
    }
    Ok(())
...
fn scan_for_suspicious_physical_write_terms() -> Result<()> {
    let suspicious_terms = ["GENERIC_WRITE", "FILE_WRITE_DATA", "raw_write", "write_at_device", "exclusive_write"];
    for entry in WalkDir::new("crates").into_iter() {
        ...
        for term in suspicious_terms {
            if text.contains(term) {
                anyhow::bail!("suspicious physical-write term `{term}` found in {}", path.display());
            }
        }
    }
    Ok(())
...
fn fixture_manifest_check(path: &Path) -> Result<()> {
    let text = fs::read_to_string(path).with_context(|| format!("read fixture manifest {}", path.display()))?;
    let manifest: JsonValue = serde_json::from_str(&text).with_context(|| format!("parse fixture manifest {}", path.display()))?;
    for field in ["schema_version", "fixture_id", "source_type", "created_with", "apfs_features", "expected_artifacts", "capability_ids", "redaction"] {
        if manifest.get(field).is_none() {
            anyhow::bail!("fixture manifest {} is missing required field `{field}`", path.display());
        }
    }
    let redaction = manifest.get("redaction").and_then(JsonValue::as_object).context("manifest redaction must be an object")?;
    if redaction.get("contains_personal_data").and_then(JsonValue::as_bool).unwrap_or(true) {
        anyhow::bail!("fixture manifest {} is marked as containing personal data", path.display());
    }
    if redaction.get("contains_secret_material").and_then(JsonValue::as_bool).unwrap_or(true) {
        anyhow::bail!("fixture manifest {} is marked as containing secret material", path.display());
    }
...
fn real_fixture_feedback(inspect_json: &Path, manifest_json: &Path, out_dir: &Path) -> Result<()> {
    ...
    if redaction.and_then(|r| r.get("contains_personal_data")).and_then(JsonValue::as_bool) != Some(false) {
        issues.push("manifest must explicitly set redaction.contains_personal_data=false".to_owned());
    }
    if redaction.and_then(|r| r.get("contains_secret_material")).and_then(JsonValue::as_bool) != Some(false) {
        issues.push("manifest must explicitly set redaction.contains_secret_material=false".to_owned());
    }

    let expected_fields = manifest.get("expected_inspect_fields").and_then(JsonValue::as_object);
    if let Some(expected_fields) = expected_fields {
        for (path, expected) in expected_fields {
            ...
            if observed == Some(expected) {
                ...
                issues.push(format!("expected `{path}` to be {expected}, observed {:?}", observed));
            }
        }
    } else {
        issues.push("manifest has no expected_inspect_fields section; feedback is generic only".to_owned());
    }

    fs::create_dir_all(out_dir)?;
    let status = if issues.is_empty() { "matched" } else { "needs_tasks" };
    ...
    fs::write(out_dir.join("real-fixture-feedback.json"), serde_json::to_string_pretty(&report)? + "\n")?;
    fs::write(out_dir.join("real-fixture-feedback.md"), format_feedback_markdown(&report))?;
...
fn format_feedback_markdown(report: &JsonValue) -> String {
    ...
    out.push_str(&format!("Status: `{}`\n\n", report.get("status").and_then(JsonValue::as_str).unwrap_or("unknown")));
    out.push_str("## Safety note\n\n");
    out.push_str(report.get("safety_note").and_then(JsonValue::as_str).unwrap_or("No safety note."));
    out.push_str("\n\n## Issues\n\n");
    if let Some(issues) = report.get("issues").and_then(JsonValue::as_array) {
        if issues.is_empty() {
            ...
            for issue in issues {
                out.push_str(&format!("- {}\n", issue.as_str().unwrap_or("<non-string issue>")));
            }
        }
    }
    out
...
fn promote_feedback(feedback_json: &Path, out_dir: &Path) -> Result<()> {
    ...
    let text = fs::read_to_string(feedback_json).with_context(|| format!("read feedback JSON {}", feedback_json.display()))?;
    let feedback: JsonValue = serde_json::from_str(&text).with_context(|| format!("parse feedback JSON {}", feedback_json.display()))?;
    let issues = feedback.get("issues").and_then(JsonValue::as_array).cloned().unwrap_or_default();
    let mut index = Vec::new();
    for (idx, issue) in issues.iter().enumerate() {
        let title = issue.get("title").and_then(JsonValue::as_str).unwrap_or("real fixture mismatch");
        let slug = title.to_ascii_lowercase().chars().map(|ch| if ch.is_ascii_alphanumeric() { ch } else { '-' }).collect::<String>();
        let slug = slug.split('-').filter(|part| !part.is_empty()).collect::<Vec<_>>().join("-");
        let track_id = format!("generated-{:04}-{}", idx + 1, if slug.is_empty() { "real-fixture-mismatch".to_owned() } else { slug });
    ...
    fs::write(out_dir.join("task-index.json"), serde_json::to_string_pretty(&index)? + "\n")?;
    println!("promote-feedback: generated {} track stubs in {}", issues.len(), out_dir.display());
    Ok(())
}


fn cargo_triage(cargo_log: &Path, out_dir: &Path) -> Result<()> {
    let args = [cargo_log.display().to_string(), out_dir.display().to_string()];
...
fn task_context(capability_id: &str) -> Result<()> {
    ...
    println!("read conductor/tracks.md and the matching conductor/tracks/<track>/spec.md + plan.md");
